...
class RecurrenceEngineTest {
    ...
    fun `TC-28 - Recurrence rules should respect frequency and intervals`() {
        ...
        val dailyStart = Calendar.getInstance().apply { set(2024, Calendar.JANUARY, 1, 10, 0, 0) }.timeInMillis
        val dailyEndRange = Calendar.getInstance().apply { set(2024, Calendar.JANUARY, 5, 23, 59, 59) }.timeInMillis
        val dailySeries = createTestSeries(id = 1, freq = RecurrenceFrequency.DAILY, start = dailyStart)
        ...
        val weeklyStart = Calendar.getInstance().apply { set(2024, Calendar.JANUARY, 1, 10, 0, 0) }.timeInMillis
        val weeklyEndRange = Calendar.getInstance().apply { set(2024, Calendar.JANUARY, 31, 23, 59, 59) }.timeInMillis
        val weeklySeries = createTestSeries(id = 2, freq = RecurrenceFrequency.WEEKLY, start = weeklyStart, interval = 2)
        
        // Étape 4 : Générer les occurrences (toutes les 2 semaines)
        val weeklyOccs = RecurrenceEngine.generateOccurrences(weeklySeries, weeklyStart, weeklyEndRange)
        ...
        val monthlyStart = Calendar.getInstance().apply { set(2024, Calendar.JANUARY, 1, 10, 0, 0) }.timeInMillis
        val monthlyEndRange = Calendar.getInstance().apply { set(2024, Calendar.DECEMBER, 31, 23, 59, 59) }.timeInMillis
        val monthlySeries = createTestSeries(id = 3, freq = RecurrenceFrequency.MONTHLY, start = monthlyStart, interval = 3)
        
        // Étape 6 : Générer les occurrences (tous les 3 mois)
        val monthlyOccs = RecurrenceEngine.generateOccurrences(monthlySeries, monthlyStart, monthlyEndRange)
    ...
    fun `TC-28 - Recurrence rules should respect endDate and isCancelled`() {
        ...
        val start = Calendar.getInstance().apply { set(2024, Calendar.JANUARY, 1, 10, 0, 0) }.timeInMillis
        val endLimit = Calendar.getInstance().apply { set(2024, Calendar.MARCH, 10, 10, 0, 0) }.timeInMillis
        val endRange = Calendar.getInstance().apply { set(2024, Calendar.DECEMBER, 31, 23, 59, 59) }.timeInMillis
        val seriesWithEnd = createTestSeries(id = 4, freq = RecurrenceFrequency.MONTHLY, start = start).copy(endDate = endLimit)
        ...
        val cancelledSeries = createTestSeries(id = 5, freq = RecurrenceFrequency.DAILY, start = start).copy(isCancelled = true)
     ...
     */
    private fun createTestSeries(id: Long, freq: RecurrenceFrequency, start: Long, interval: Int = 1) = RecurringSeriesEntity(
        id = id,
        title = "Test Series",
        amount = 100.0,
        type = TransactionType.EXPENSE,
